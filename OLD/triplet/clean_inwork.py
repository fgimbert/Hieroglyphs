...
train_data=[dataset[0][:ntrain].reshape((-1,img_height,img_width, 3)),dataset[1][:ntrain].reshape((-1,img_height,img_width, 3)),dataset[2][:ntrain].reshape((-1,img_height,img_width, 3))]
test_data=dataset[0][ntrain:].reshape((-1,img_height,img_width, 3))
...
history = loss_model.fit(train_data,batch_size=16,epochs=100,verbose=2)
...
for i in range(9):
    ...
    ax.imshow(test_data[i+5].reshape(img_height, img_width,3),cmap='gray')
    ax.set_xticks([])
    ax.set_yticks([])
    fig.add_subplot(ax)
    ax = plt.Subplot(fig, inner_grid[1])
    index=dico_hiero[hieroglyph][1][1]
    ax.imshow(train_data[0][index].reshape(img_height, img_width,3),cmap='gray')
    ax.set_xticks([])
    ax.set_yticks([])
...
fig.savefig('../screenshots/results_test.png')
